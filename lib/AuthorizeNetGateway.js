...
function setRequest (service, rootNodeName, requestNode) {
  ...
  if (requestNode.key)
    ...
    for (var key in requestNode.value)
      requestObject[rootNodeName][key] = requestNode.value[key];
  }
...
AuthorizeNetGateway.prototype.createCustomerProfile = function (payment, billing, shipping, options) {
  ...
  return P.resolve()
    .then(function () {
      ...
      if (payment.cvv2)
        body.paymentProfiles.payment.creditCard.cardCode = payment.cvv2;
 ...
 *
 */
AuthorizeNetGateway.prototype.createCustomerPaymentProfileRequest = function (customerProfileId, payment, billing, options) {
  ...
  return P.resolve()
    .then(function () {
      ...
      if (payment.cvv2)
        body.paymentProfile.creditCard.cardCode = payment.cvv2;

      var obj = setRequest(service, 'createCustomerPaymentProfileRequest', {
        key: '',
        value: body
      });

      obj.createCustomerPaymentProfileRequest.validationMode = this.testMode ? 'testMode' : 'none';
    ...
    .spread(createJsonCallback(function (json) {
      ...
      if (json.createCustomerPaymentProfileResponse) {

        if (json.createCustomerPaymentProfileResponse.messages[0].resultCode[0] === 'Error') {

          errors = json.createCustomerPaymentProfileResponse.messages[0].message[0];
          errors = errors.length > 1 ? errors : [errors];

          throw new GatewayError(errors.reduce(function (previous, current) {
            previous += '- ' + current.text;
            return previous;
          }, ''), json.createCustomerPaymentProfileResponse);
        }

        return {
          profileId: json.createCustomerPaymentProfileResponse.customerPaymentProfileId[0],
          _original: json.createCustomerPaymentProfileResponse
        };
    ...
    .then(function (res) {
      ...
      var body = {
        ...
        profile: {
          ...
          paymentProfile: {
            paymentProfileId: other.paymentProfileId || paymentProfile[0][0].customerPaymentProfileId[0]
          }
        }
      };
      if (other.order)
        body.order = other.order;
      if (other.lineItems)
        body.lineItems = other.lineItems.lineItem ? other.lineItems : {
          lineItem: other.lineItems
        };
      if (other.tax)
        body.tax = other.tax;
