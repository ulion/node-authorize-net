...
AuthorizeNetGateway.prototype.getSettledBatchList = function getSettledTransactionsList (from, to) {
  ...
  if (isNaN(fromDate)) {
    return P.reject(new Error('From required.'));
  }
    ...
    .spread(createJsonCallback(function (json) {
      ...
      if (json.getSettledBatchListResponse) {
        ...
        if (json.getSettledBatchListResponse.messages[0].resultCode[0] === 'Error') {
          ...
          var errors = json.getSettledBatchListResponse.messages[0].message[0];
        ...
        return batchList[0][0].batch.map(function (batch) {
          var output = {
            batchId: batch.batchId[0],
            settlementDate: new Date(batch.settlementTimeUTC[0]),
            paymentMethod: batch.paymentMethod[0],
            marketType: batch.marketType[0],
          ...
          var statsArray = batch.statistics && batch.statistics[0] && batch.statistics[0].statistic;

          if (statsArray) {
            output._statistics = statsArray.map(function (stat) {
              var parsed = {
                accountType: stat.accountType[0],
                chargeAmount: parseFloat(stat.chargeAmount[0]),
                refundAmount: parseFloat(stat.refundAmount[0]),
                chargeCount: parseInt(stat.chargeCount[0]),
                refundCount: parseInt(stat.refundCount[0]),
                voidCount: parseInt(stat.voidCount[0]),
                declineCount: parseInt(stat.declineCount[0]),
                errorCount: parseInt(stat.errorCount[0]),
              };

              output.chargeAmount += (parsed.chargeAmount || 0);
              output.refundAmount += (parsed.refundAmount || 0);
              output.chargeCount += (parsed.chargeCount || 0);
              output.refundCount += (parsed.refundCount || 0);
              output.voidCount += (parsed.voidCount || 0);
              output.declineCount += (parsed.declineCount || 0);
              output.errorCount += (parsed.errorCount || 0);
...
AuthorizeNetGateway.prototype.getTransactionList = function getTransactionList(batchId) {
  if (!batchId) {
    return P.reject(new Error('Batch ID required.'));
  }
    ...
    .spread(createJsonCallback(function (json) {
      ...
      if (json.getTransactionListResponse) {
        ...
        return transactions.map(function (transaction) {
          var parsed = {
            transactionId: transaction.transId[0],
            submitTime: new Date(transaction.submitTimeUTC[0]),
            transactionStatus: transaction.transactionStatus[0],
            accountType: transaction.accountType[0],
            accountNumber: transaction.accountNumber[0],
            settleAmount: parseFloat(transaction.settleAmount[0]),
            marketType: transaction.marketType[0],
            product: transaction.product[0],
          };

          var subscription = transaction.subscription && transaction.subscription[0];
          if (subscription) parsed.subscription = transaction.subscription[0];

          var firstName = transaction.firstName && transaction.firstName[0];
          if (firstName) parsed.firstName = transaction.firstName[0];

          var lastName = transaction.lastName && transaction.lastName[0];
          if (lastName) parsed.lastName = transaction.lastName[0];
